    ...
    return (
        <div className={cn("fixed bottom-6 z-50 flex items-center gap-4", isRTL ? "left-6" : "right-6")}>
            <AnimatePresence>
                {showFloatingMessage && !isOpen && (
                    <motion.div
                        ...
                        className={cn(
                            "bg-white shadow-lg rounded-2xl p-4 max-w-[200px] border border-orange-100 hidden md:block relative",
                            isRTL ? "order-last ml-2" : "mr-2"
                        )}
                    >
                        <div className={cn(
                            "absolute top-1/2 -translate-y-1/2 w-3 h-3 bg-white border-t border-r border-orange-100 rotate-45",
            ...
            <motion.button
                ...
                onClick={onOpen}
                className={cn(
                    "p-4 rounded-full shadow-2xl text-white bg-gradient-to-r from-primary to-accent-600 border-2 border-orange-200/50 backdrop-blur-sm relative group",
                    isOpen ? "hidden" : "flex"
                )}
            >
                <div className="absolute inset-0 bg-[url('/assets/images/hieroglyphs-bg.png')] opacity-20 bg-cover mix-blend-overlay rounded-full" />
